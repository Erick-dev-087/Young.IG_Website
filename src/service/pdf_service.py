"""
PDF Service — Young IG Auto Inspection System
...
# Inspects the string value from an inspector and decides the severity tier.
# ---------------------------------------------------------------------------
...
def _build_category_sections(
    ...
    """
    For each category render a side-by-side block:
      Left 65% : findings table (with tinted rows + pill badges)
      Right 35% : thumbnail grid of photos for that same category
    """
    if not categories:
    ...
    for idx, cat in enumerate(categories, start=1):
        ...
        for finding in cat.findings:
            ...
            badge_html = _badge(finding.value)
            rows_html += f"""
                <tr class="{row_class}">
                    <td class="field-name">{finding.field_name}</td>
                    <td class="field-value">{badge_html}{notes_html}</td>
                </tr>
            """
        ...
        # ── Category-specific photo grid ────────────────────────────────────
        # Try to match by category name (case-insensitive) or fall back to empty
        cat_key = cat.category_name.upper()
        cat_images = images_by_category.get(cat_key, [])

        if cat_images:
            thumbs = ""
            for img in cat_images[:4]:  # max 4 thumbs per category
                thumbs += f"""
                    <div class="thumb-card">
                        <img src="{img.image_url}" alt="{cat.category_name}" />
                    </div>
                """
...
def _build_hero_image(images_by_category: Dict[str, List[PDFImage]]) -> str:
    """Pick the best hero image: first EXTERIOR, else any image available."""
    hero_url = ""
...
def _build_summary_panel(report: InspectionPDFReport) -> str:
    """The executive summary panel replacing the old bare banner."""
...
def _render_html(report: InspectionPDFReport) -> str:
    ...
    <head>
        ...
        <style>
            ...
            * {{ margin: 0; padding: 0; box-sizing: border-box; }}
            ...
            .findings-table tbody tr.row-fail {{
                background: #fff1f2 !important;
            }}
            .findings-table tbody tr.row-warn {{
    ...
    <body>
            ...
            <div class="hero-specs">
                ...
                <div class="specs-grid">
                    <div class="spec-item">
                        ...
                        <span class="spec-value">{v.registration_number}</span>
                    </div>
                    <div class="spec-item">
                        <span class="spec-label">Make</span>
                        <span class="spec-value">{v.make or "—"}</span>
                    </div>
                    <div class="spec-item">
                        <span class="spec-label">Model</span>
                        <span class="spec-value">{v.model or "—"}</span>
                    </div>
                    <div class="spec-item">
                        <span class="spec-label">Year</span>
                        <span class="spec-value">{v.manufacture_year or "—"}</span>
                    </div>
                    <div class="spec-item">
                        <span class="spec-label">Fuel Type</span>
                        <span class="spec-value">{v.fuel_type or "—"}</span>
                    </div>
                    <div class="spec-item">
                        <span class="spec-label">Transmission</span>
                        <span class="spec-value">{v.transmission or "—"}</span>
                    </div>
                    <div class="spec-item">
                        <span class="spec-label">VIN / Chassis</span>
                        <span class="spec-value">{v.chassis_number or "—"}</span>
                    </div>
                    <div class="spec-item">
                        <span class="spec-label">Engine No.</span>
                        <span class="spec-value">{v.engine_number or "—"}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- ═══ INSPECTION FINDINGS (side-by-side) ══════════════════════ -->
        {category_sections}
            ...
            <div class="meta-grid">
                <div class="meta-col">
                    ...
                    <div class="meta-item">
                        ...
                        <span class="meta-value">{inspector.full_name}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Email</span>
                        <span class="meta-value">{inspector.email}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Phone</span>
                        <span class="meta-value">{inspector.phone or "—"}</span>
                    </div>
                </div>
                <div class="meta-col">
                    <h4>Customer</h4>
                    <div class="meta-item">
                        <span class="meta-label">Name</span>
                        <span class="meta-value">{cust_name}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Phone</span>
                        <span class="meta-value">{cust_phone}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Email</span>
                        <span class="meta-value">{cust_email}</span>
                    </div>
                </div>
                <div class="meta-col">
                    <h4>Seller / Dealer</h4>
                    <div class="meta-item">
                        <span class="meta-label">Seller Name</span>
                        <span class="meta-value">{seller}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Inspection No.</span>
                        <span class="meta-value">{report.inspection_number}</span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Inspection Date</span>
                        <span class="meta-value">{report.inspection_date.strftime('%d %B %Y')}</span>
                    </div>
                </div>
            </div>
        </div>
...
class PDFService:
    ...
    Windows dev machines that don't have GTK+ installed.
    """
